...
data_main <- data %>% 
  ...
  select(yardline_100, game_seconds_remaining, qtr, 
         ...
         week, posteam, posteam_type, play_type, wpa, pos_team_epa) %>%
  filter(down == 4) %>% 
...
The goal with our models is to create the best possible models for predicting WPA (win probability added) given a certain game state. Because these models will not be used for any inference, 
...
punt_model = xgb.train(data = punt_xgb_train, max_depth = 6,  colsample_bylevel = 1, colsample_bytree = 1, watchlist=punt_watchlist, nrounds = 1500, verbose = 0, eta = 0.1, subsample = 0.9)
punt_model_xgboost = xgboost(data = punt_xgb_train, max.depth = 6, nrounds = 1475, colsample_bytree = 1, colsample_bylevel = 1, verbose = 0, eta = 0.1, subsample = 0.9)
...
go_model = xgb.train(data = go_xgb_train, max_depth = 6,  colsample_bylevel = 1, colsample_bytree = 1, watchlist=kick_watchlist, nrounds = 1000, verbose = 1, eta = 0.1, subsample = 0.9)
go_model_xgboost = xgboost(data = go_xgb_train, max_depth = 6,  colsample_bylevel = 1, colsample_bytree = 1, nrounds = 60, verbose = 1, eta = 0.1, subsample = 0.9)
...
punt_preds = predict(punt_model_xgboost, xgb_matrix)
kick_preds = predict(kick_model_xgboost, xgb_matrix)
go_preds = predict(go_model_xgboost, xgb_matrix)

pred_data = cbind(pred_data, play_type = data_main$play_type, punt_preds, kick_preds, go_preds) %>% 
  mutate(recommendation = case_when((punt_preds > kick_preds) & (punt_preds > go_preds) ~ "punt",
                                    (kick_preds > punt_preds) & (kick_preds > go_preds) ~ "field_goal",
                                    (go_preds > kick_preds) & (go_preds > punt_preds) ~ "go"),
         correct_call = ifelse(play_type == recommendation, "Yes", "No")) 

pred_data %>% 
  filter(correct_call == "No") %>% 
  select(yardline_100, game_seconds_remaining, ydstogo, score_differential, play_type, recommendation)
```
