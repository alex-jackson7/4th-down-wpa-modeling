...
Most 4th down plays are punts, of course, and only a small piece of our dataset is go for it plays, which also makes sense. Looking at the graph above, however, we can see an uptick in go attempts and a steady decrease in punts over the last 4 full seasons. This hints at the greater picture we are trying to discover: coaches haven't found the right go/punt/kick balance. But they are trending in the right direction. 

```{r count by decision, echo = FALSE, fig.width=7, fig.height=4}
...
While our models and process did not turn out exactly how we wanted it to, we still ended up with a working product and learned a lot while making it.
